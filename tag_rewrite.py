...
def replace_yaml_tags(yaml_block, tag_map, changes):
    try:
        ...
        if parsed and isinstance(parsed, dict) and 'tags' in parsed:
            ...
            if isinstance(tags, list):
                ...
                for old, new in zip(tags, updated):
                    if old != new:
                        changes.append(f"  YAML tag was: {old}  ->  tag new: {new}")
...
def update_file_tags(md_path, tag_map, dry_run=False):
    ...
    if original.startswith("---"):
        ...
        if end != -1:
            ...
            fixed_yaml = replace_yaml_tags(yaml_block, tag_map, changes)
    ...
    def repl(m):
        ...
        if tag != new_tag:
            changes.append(f"  inline tag was: {tag}  ->  tag new: {new_tag}")
        return f"#{new_tag}"
    ...
    if changed:
        if dry_run:
            ...
            if changes:
                ...
                print("  [NO TAG CHANGES detected — unexpected update? Investigate.]")
        else:
            ...
            if changes:
                ...
                print("  [NO TAG CHANGES detected — updated anyway. Investigate this case.]")
